...
## 2026-03-29 — Session 5 — V2 UX Redesign (3-tab), Supplements CRUD, Labs filters
## 2026-03-29 — Session 4 — Connect Devices, 4 new HealthKit types, Oura OAuth fix, Labs view
## 2026-03-29 — Session 3 (continued) — Polish pass (haptics, skeletons, animations, offline), Oura integration
## 2026-03-29 — Session 3 — First compile, Bearer token auth fix, API model mismatches, stale token fix

*(Full details for sessions 3-5 were archived to reduce changelog size. See git history for complete entries.)*

## 2026-03-28 — Session 2

### Bug Fix: SyncService Wiring
- **Found:** SyncStatusView was creating its own throwaway SyncService with dummy AuthService/HealthKitService instances. The "Sync Now" button was syncing with an unauthenticated, disconnected service — data never flowed properly.
- **Fix:** Created single SyncService in VitalApp.init() alongside AuthService and HealthKitService, passed down via .environmentObject(). All views and background tasks now share the same instance.

### Bug Fix: Duplicate HealthKit Data (Steps/Calories/Exercise Inflated ~2x)
- **Found:** `HKSampleQuery` returns raw samples from ALL sources (iPhone + Apple Watch). Summing raw samples double-counts overlapping periods.
- **Fix:** Switched cumulative metrics (steps, exercise, calories) to `HKStatisticsCollectionQuery` with `.cumulativeSum`, which deduplicates across sources automatically — same as Apple's Health app.
- Also fixed SyncService to query from `startOfDay(lastSyncDate)` instead of exact lastSyncDate, so cumulative metrics always contain full-day totals.

### Bug Fix: Backend Ingest Overwriting Daily Totals
- **Found:** Supabase `.upsert()` replaced existing daily metric rows on re-sync, causing partial-day data to overwrite correct totals.
- **First attempt:** Added cumulative add-to-existing logic — caused double-counting when same samples were re-sent. Reverted.
- **Final fix:** iOS app sends full-day totals (via HKStatisticsCollectionQuery), backend simply replaces. Each sync writes correct current totals.

### Bug Fix: Memory Crash on Sync
- **Found:** App terminated by OS for excessive memory (code 9). Caused by per-day `HKStatisticsQuery` loop + unlimited sample queries.
- **Fix:** Replaced per-day loop with single `HKStatisticsCollectionQuery`. Capped discrete metrics at 500 samples, sleep at 200, workouts at 100.

### Error Handling + Retry Logic
- Added exponential backoff retry (3 attempts: 2s, 4s, 8s) for transient failures (5xx, timeouts)
- 401 errors auto-refresh Supabase session and retry once before failing
- Network errors show specific messages (no internet, timeout, connection lost)
- Added HealthKit `authorizationDenied` error case
- Added `refreshSession()` to AuthService

### Features Added
- **Force Full Sync** button in Settings — resets lastSyncDate and does 7-day lookback
- **Launch screen** — dark background (#0A0A0C) with Assets catalog
- **Privacy policy** — hosted at /privacy on dashboard (public, no auth required)

### App Store Submission Checklist
- Added new Phase 3 items: App Store description, demo account, "Works with Apple Health" badge, iOS 26 SDK upgrade

### Files Changed (iOS)
- `Vital/VitalApp.swift` — Single shared SyncService, proper StateObject init
- `Vital/Views/SyncStatusView.swift` — Uses @EnvironmentObject instead of own SyncService
- `Vital/Views/SettingsView.swift` — Added Force Full Sync button
- `Vital/Services/SyncService.swift` — Retry logic, start-of-day queries, resetAndSync()
- `Vital/Services/AuthService.swift` — Added refreshSession()
- `Vital/Services/HealthKitService.swift` — HKStatisticsCollectionQuery, sample limits, authorizationDenied error
- `Vital/Info.plist` — Launch screen config
- `Vital.xcodeproj/project.pbxproj` — Assets catalog + Resources build phase

### Files Changed (Dashboard)
- `src/app/api/ingest/apple-health/route.ts` — iOS token auth, sleep accumulation fix, replace strategy for upserts
- `src/components/layout/AppShell.tsx` — Privacy page excluded from nav chrome
- `src/app/privacy/page.tsx` — New privacy policy page

### Files Created
- `Vital/Assets.xcassets/` — Asset catalog with LaunchBackground color + AppIcon placeholder

### Decisions
- Labs are not synced via iOS app (not in HealthKit) — web dashboard only
- Launch screen is minimal dark bg; logo swap when asset is ready
- App icon deferred to later session
- Cumulative metrics use HKStatisticsCollectionQuery (deduplicates iPhone + Watch sources)
- Backend uses replace strategy for daily metrics (iOS sends full-day totals each sync)

### Status
Phase 1: **Complete**
Phase 2: 4/5 done (background sync test remaining)
Phase 3: 6/12 done

### What's Next
- App icon (waiting on logo/asset)
- App Store description + demo account for reviewer
